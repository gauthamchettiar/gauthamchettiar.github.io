...
A sample snippet of dataset I have been trying to transform -
...
If you want to follow along with me, here's a json equivalent for you to download - [products.json](/code/20230724-confessing-my-love-for-duckdb/products.json)
...
One thing that really sold me the idea of using DuckDB was of course that all queries are in SQL and it is blazing-ly fast. In some instances I have seen it process sufficiently larger dataset faster than spark on databricks [^1]. 
...
I will not lie, to understand you might need to know some of DuckDB's features. But even without any, you should be able to make something out of it. If not, here's how it works -

==`price.*`== in inner sub-query (line 6) will automatically flatten each individual keys into new column, with value as entry.  
...
{width="full", caption="Output of inner subquery (line 5-7)"  align="left"}
...
| 102          | Bangalore |                   |
{width="full", caption="Output of outer subquery (line 4-8)" align="left"}

==`EXCLUDE`== can be used if you want to select everything except some columns :  
1. on line 8, used to tell duckdb to unpivot on all columns except product_code
2. on line 1, used to tell duckdb to include all columns except price, which we are anyway flattening out

Ain't that a beauty. Now once, you learn this, it's very difficult to forget. You can come back to this code a year later, and at least some part of it might still make sense. Unlike the spaghetti of a code from Pandas [^2].
...
[^1]: This is quite a loose comparison. Even though dataset was same, they were both running on different machines with almost similar configuration. So it's not an apples to apples comparison, more so like apple to pear.

[^2]: Complete code for both examples: [Pandas Code](/code/20230724-confessing-my-love-for-duckdb/flatten.py) & [DuckDB SQL](/code/20230724-confessing-my-love-for-duckdb/flatten.sql)